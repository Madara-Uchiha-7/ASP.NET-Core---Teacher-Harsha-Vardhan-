...
It operates on a client-server model, where the client (usually a web browser) makes requests to a server, 
which then responds with the requested resources or error messages.

Stateless Protocol: Each HTTP request is independent of others; the server does not retain information 
...
An HTTP server is software that handles HTTP requests from clients and serves back responses. 
It processes incoming requests, executes the necessary logic (e.g., accessing a database, generating HTML), 
...
When a user enters a URL or clicks a link, the browser sends an HTTP 
request to the server, which then responds with the requested resource.
...
context.Response.StatusCode = 400;: Sets the status code to 400 Bad Request 
(this line won't be executed due to the condition).
...
Summary
HTTP Protocol: A fundamental protocol for web communication, 
following a request/response model and operating statelessly.
...
Status Codes: Indicate the result of HTTP requests, categorized into informational, success, 
redirection, client error, and server error codes.

Setting Status Codes and Headers: ASP.NET Core allows customization of responses using code, 
enabling setting of status codes and headers as demonstrated.
...
HTTP Requests
In the world of web applications, an HTTP request is a client's way of saying, "Hey server, 
I need something." This "something" could be a web page, an image, data from a database, 
or the result of some server-side calculation. The client, typically a web browser, 
sends this request to the server, which processes it and returns a response.
...
Request URI (Uniform Resource Identifier): This is the path to the resource on the server that the 
...
Body (Optional): This part of the request contains data that the client is 
sending to the server. For example, a POST request might include form data or JSON data.
...
A query string is a way to pass parameters to a server within the URL itself. 
It starts with a question mark (?) and follows the path in the URL. Each parameter is a key-value pair, 
...
ASP.NET Core provides a HttpRequest object that gives you access to all the 
information within an incoming request. This object has properties like:
...
If found, it extracts the value of the User-Agent header and displays it, 
indicating the client's browser or application.
...
HTTP requests are the messages sent from clients (like web browsers) to servers to request resources or actions. 
They consist of a start line (method, URI, HTTP version), headers (additional information), an empty line, 
and an optional body containing data. Query strings are used to pass parameters within URLs.
...
The GET method is primarily designed for fetching data from a server. 
Think of it as asking the server for a specific resource, 
like a webpage, an image, or some data from a database. Here's what characterizes GET requests:

Data in the URL: Parameters are appended to the URL as a query string. 
This makes the request parameters visible in the browser's address bar.

Limited Data Size: The size of data that can be sent in a GET request is restricted due to limitations 
in URL lengths (browsers and servers might have different limits).

Idempotent: GET requests are considered idempotent. This means you can make the same GET 
request multiple times, and it should have the same effect as making it once 
(assuming the underlying data hasn't changed).

Caching: GET requests can be cached, meaning that if a client requests the same resource again, 
the browser might serve the previously retrieved response from its cache, improving performance.

Security: GET requests are generally less secure than POST requests because the data is visible 
in the URL. Avoid using GET for sensitive information like passwords or credit card numbers.
...
The POST method is primarily used for submitting data to the server for processing. 
This data is typically included in the body of the request and is not visible in the URL. 
Here's how POST requests differ from GET:

Data in the Body: Data is sent in the request body, 
making it more suitable for sending large amounts of data or sensitive information.

Not Idempotent: POST requests are not idempotent. Repeated POST requests 
might result in different outcomes (e.g., creating multiple resources or triggering actions multiple times).

Not Cachable: POST requests are generally not cached, as they often result in changes on the server.

Security: POST requests are considered more secure than GET requests because 
the data is not exposed in the URL. However, they are still susceptible to 
attacks like cross-site request forgery (CSRF), which requires additional security measures.
...
Postman
Postman is a versatile API development and testing tool. It allows you to easily craft 
HTTP requests, send them to your ASP.NET Core application (or any API), and inspect the 
responses. It's a fantastic way to debug, experiment, and explore your API endpoints.

Installation

Download: Head to the official Postman website (https://www.postman.com/downloads/) and 
...
Let's say your ASP.NET Core application is running locally at https://localhost:7070 and 
has an endpoint /api/products. Here's how to use Postman:
...
If your endpoint requires specific headers (like Content-Type), click on the "Headers" tab and 
...
Foundation of the Web: HTTP is the protocol that powers the World Wide Web. 
It defines how clients (browsers, apps) and servers communicate.

Request-Response Cycle: Communication follows a request-response model. 
...
Servers don't inherently remember past interactions.
